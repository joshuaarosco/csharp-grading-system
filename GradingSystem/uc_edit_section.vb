...
Public Class uc_edit_section
    ...
    Public Property AdviserName As String

    Private Sub uc_edit_section_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        load_courses()
        load_teachers()
    End Sub
    ...
    Private Sub update_data()
        ...
        Try
            MysqlConn.Open()
            Dim Query As String
            Query = "update grading_system.sections set course = '" & cb_course.SelectedValue & "', year_level = '" & cb_year_level.SelectedValue & "', section_name = '" & txt_section.Text & "',adviser_id = '" & cb_teacher.SelectedValue & "', adviser_name = '" & cb_teacher.Text & "' ,school_year = '" & mtxt_school_year.Text & "', created_at = '" & DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") & "' where id = '" & SectionId & "'"
            MyCommand = New MySqlCommand(Query, MysqlConn)
            Reader = MyCommand.ExecuteReader
            MysqlConn.Close()
        Catch ex As Exception
